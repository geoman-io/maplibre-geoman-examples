...
const GROUPS: Array<{ name: string; tools: Tool[] }> = [
    ...
    tools: [
      {
        id: 'split_plane',
        icon: 'split',
        title: 'Split roof plane',
        hint: 'Split — draw a line across a roof plane to divide it',
        run: (gm) => gm.enableMode('edit', 'split' as never),
      },
      {
        id: 'merge_planes',
        icon: 'union',
        title: 'Merge roof planes',
        hint: 'Merge — click two adjoining roof planes to combine them',
        run: (gm) => gm.enableMode('edit', 'union' as never),
      },
      {
        id: 'simplify',
        icon: 'simplify',
        title: 'Clean up outline',
        needsSelection: true,
        hint: 'Simplify — reduce redundant vertices of a roof or panel traced from imagery',
  ...
  },
  {
    name: 'Draw',
    tools: [
      draw('polygon', 'polygon', 'Roof / obstruction'),
      draw('rectangle', 'rectangle', 'Panel / array'),
    ],
  },
  {
    name: 'Edit',
...
export default function Toolbar({ gm, controller }: { gm: Geoman; controller: EditorController }) {
  ...
  return (
    <div className="pointer-events-auto flex w-max items-center gap-0.5 p-1">
